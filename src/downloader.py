...
from .exceptions import (
    DownloadCancelled,
)  # <-- استيراد نسبي للاستثناء Relative import for exception
from .logic_utils import (
    clean_filename,
)  # <-- استيراد دالة التنظيف من ملف الأدوات Import clean function from utils
...
class Downloader:
    ...
    def __init__(
        ...
        self._last_hook_playlist_index = (
            0  # آخر فهرس مطلق تم رؤيته في الهوك Last absolute index seen in hook
        )
    ...
    def _my_hook(self, d):
        ...
        hook_playlist_index = info_dict.get(
            "playlist_index"
        )  # الفهرس المطلق (يبدأ من 1) Absolute index (1-based)
        ...
        if status == "finished":
            ...
            if filepath:
                ...
                print(
                    f"Hook 'finished' (pre-postprocessing): Path reported '{filepath}'."
                )
                ...
                self.progress_callback(
                    1.0
                )  # التقدم 100% لهذه المرحلة Progress 100% for this stage
            else:
            ...
            if downloaded_bytes is not None:
                ...
                if self.is_playlist:
                    ...
                    )  # لا يتجاوز العدد المحدد Don't exceed selected count
                    remaining_in_selection = max(
                        0, self.selected_items_count - self._processed_selected_count
                    )  # لا يقل عن صفر Cannot be less than zero
                    status_lines.append(
                ...
                ):  # تجاهل أخطاء التحويل المحتملة Ignore potential conversion errors
    ...
    def _download_core(self):
        ...
        ydl_opts = {
            "progress_hooks": [
                self._my_hook
            ],  # خطاف التقدم (قبل المعالجة اللاحقة) Progress hook (before postprocessing)
        ...
        if self.ffmpeg_path:
            ...
            core_postprocessors
        ):  # تحذير إذا كان مطلوبًا ولم يوجد Warning if needed but not found
        ...
        if self.playlist_items:
            ydl_opts["playlist_items"] = (
                self.playlist_items
            )  # تحديد العناصر المطلوبة Specify items
        else:
            ydl_opts["noplaylist"] = (
                True  # تعطيل معالجة القائمة Disable playlist processing
            )
